...
/**
 * Wrapper function for the Omega_Media_Meta class.
 *
 * @since  0.9.0
 * @access public
 * @param  array   $args
 * @return string
 */
function omega_media_meta( $args = array() ) {

	$meta = new Omega_Media_Meta( $args );
...
class Omega_Media_Meta {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'omega_media_meta_args', wp_parse_args( $args, $defaults ) );
		...
		elseif ( omega_attachment_is_audio( $this->args['post_id'] ) )
			$this->audio_meta();

		/* If the attachment is video. */
		elseif ( omega_attachment_is_video( $this->args['post_id'] ) )
			$this->video_meta();
	}
	 ...
	 * function for that called `omega_get_audio_transcript()`.
	 *
	 * @since  0.9.0
	 * @access public
	 * @return void
	...
	public function audio_meta() {
		...
		if ( !empty( $this->meta['length_formatted'] ) )
			$this->items['length_formatted'] = array( $this->meta['length_formatted'], __( 'Run Time', 'omega' ) );

		/* Artist. */
		if ( !empty( $this->meta['artist'] ) )
			$this->items['artist'] = array( $this->meta['artist'], __( 'Artist', 'omega' ) );

		/* Composer. */
		if ( !empty( $this->meta['composer'] ) )
			$this->items['composer'] = array( $this->meta['composer'], __( 'Composer', 'omega' ) );

		/* Album. */
		if ( !empty( $this->meta['album'] ) )
			$this->items['album'] = array( $this->meta['album'], __( 'Album', 'omega' ) );

		/* Track number (should also be an album if this is set). */
		if ( !empty( $this->meta['track_number'] ) )
			$this->items['track_number'] = array( $this->meta['track_number'], __( 'Track', 'omega' ) );

		/* Year. */
		if ( !empty( $this->meta['year'] ) )
			$this->items['year'] = array( date_i18n( 'Y', $this->meta['year'] ), __( 'Year', 'omega' ) );

		/* Genre. */
		if ( !empty( $this->meta['genre'] ) )
			$this->items['genre'] = array( $this->meta['genre'], __( 'Genre', 'omega' ) );

		/* File name.  We're linking this to the actual file URL. */
		$this->items['file_name'] = array( '<a href="' . esc_url( wp_get_attachment_url( $this->args['post_id'] ) ) . '">' . basename( get_attached_file( $this->args['post_id'] ) ) . '</a>', __( 'File Name', 'omega' ) );

		/* File size. */
		if ( !empty( $this->meta['filesize'] ) )
			$this->items['filesize'] = array( size_format( $this->meta['filesize'], 2 ), __( 'File Size', 'omega' ) );
		...
		if ( preg_match( '/^.*?\.(\w+)$/', get_attached_file( $this->args['post_id'] ), $matches ) )
			$this->items['file_type'] = array( esc_html( strtoupper( $matches[1] ) ), __( 'File Type', 'omega' ) );

		/* Mime type. */
		if ( !empty( $this->meta['mime_type'] ) )
			$this->items['mime_type'] = array( $this->meta['mime_type'], __( 'Mime Type', 'omega' ) );
	...
	public function video_meta() {
		...
		if ( !empty( $this->meta['width'] ) && !empty( $this->meta['height'] ) )
			/* Translators: Media dimensions - 1 is width and 2 is height. */
			$this->items['dimensions'] = array( sprintf( __( '%1$s &#215; %2$s', 'omega' ), number_format_i18n( absint( $this->meta['width'] ) ), number_format_i18n( absint( $this->meta['height'] ) ) ), __( 'Dimensions', 'omega' ) );
