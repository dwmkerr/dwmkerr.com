<?php
/**
 * Omega Media Grabber - A script for grabbing media related to a post.
 *
 * Omega Media Grabber is a script for pulling media either from the post content or attached to the 
...
/**
 * Wrapper function for the Omega_Media_Grabber class.  Returns the HTML output for the found media.
 *
 * @since  0.9.0
 * @access public
 * @param  array
 * @return string
 */
function omega_media_grabber( $args = array() ) {

	$media = new Omega_Media_Grabber( $args );
...
class Omega_Media_Grabber {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'omega_media_grabber_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'omega_media_grabber_autoembed_media',       array( $wp_embed, 'autoembed' ) );
		...
		$this->args    = apply_filters( 'omega_media_grabber_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'omega_media_grabber_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'omega_media_grabber_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'omega_media_grabber_autoembed_media', $value[0] ) );
	...
	public function filter_dimensions( $html ) {
		...
		list( $width, $height ) = apply_filters( 
			'omega_media_grabber_dimensions', 
