 ...
 * @return void
 */
function omega_set_prefix( $prefix ) {
	global $omega;

	$omega->prefix = sanitize_key( apply_filters( 'omega_prefix', $prefix ) );
}

/**
 * Defines the theme prefix. This allows developers to infinitely change the theme. In theory,
 * one could use the Omega core to create their own theme or filter 'omega_prefix' with a 
 ...
 * @access public
 * @global object $omega         The global Omega object.
 * @return string $omega->prefix The prefix of the theme.
 */
function omega_get_prefix() {
	global $omega;

	/* If the global prefix isn't set, define it. Plugin/theme authors may also define a custom prefix. */
	if ( empty( $omega->prefix ) )
		$omega->prefix = sanitize_key( apply_filters( 'omega_prefix', get_template() ) );
...
function omega_format_hook( $tag, $context = '' ) {
	return omega_get_prefix() . ( ( !empty( $context ) ) ? "_{$context}" : "" ). "_{$tag}";
}
