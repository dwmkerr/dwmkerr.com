 ...
 * use of previous Omega Core functionality related to post formats as well as fixing the backwards-
...
add_action( 'wp_loaded', 'omega_structured_post_formats', 1 );
 ...
 * @return string
 */
function omega_clean_post_format_slug( $slug ) {
 ...
 * @return string $content
 */
function omega_link_content( $content ) {
 ...
 * @return string $content
 */
function omega_quote_content( $content ) {
 ...
 * @return array
 */
function omega_get_the_post_format_chat( $content ) {

	/* Allow the separator (separator for speaker/text) to be filtered. */
	$separator = apply_filters( 'omega_post_format_chat_separator', ':' );
 ...
 * @since  0.9.0
 * @access public
 * @global array   $_omega_post_chat_ids  An array of IDs for the chat rows based on the author.
 * @param  string  $content                The content of the post.
 * @return string  $chat_output            The formatted content of the post.
 */
function omega_chat_content( $content ) {
	...
	$stanzas = omega_get_the_post_format_chat( $content );
	...
	foreach ( $stanzas as $stanza ) {
		...
		foreach ( $stanza as $row ) {
			...
			$speaker_id = omega_chat_row_id( $chat_author );
			...
			if ( !empty( $chat_author ) )
				$chat_output .= "\n\t\t\t\t\t" . '<div class="chat-author ' . sanitize_html_class( strtolower( "chat-author-{$chat_author}" ) ) . ' vcard">' . $time . '<cite class="fn">' . apply_filters( 'omega_post_format_chat_author', $chat_author, $speaker_id ) . '</cite>:</div>';

			/* Add the chat row text. */
			$chat_output .= "\n\t\t\t\t\t" . '<div class="chat-text">' . str_replace( array( "\r", "\n", "\t" ), '', apply_filters( 'omega_post_format_chat_text', $chat_text, $chat_author, $speaker_id ) ) . '</div>';
 ...
 */
function omega_chat_row_id( $chat_author ) {
	global $_omega_post_chat_ids;

	/* Let's sanitize the chat author to avoid craziness and differences like "John" and "john". */
	$chat_author = strtolower( strip_tags( $chat_author ) );

	/* Add the chat author to the array. */
	$_omega_post_chat_ids[] = $chat_author;

	/* Make sure the array only holds unique values. */
	$_omega_post_chat_ids = array_unique( $_omega_post_chat_ids );

	/* Return the array key for the chat author and add "1" to avoid an ID of "0". */
	return absint( array_search( $chat_author, $_omega_post_chat_ids ) ) + 1;
}
