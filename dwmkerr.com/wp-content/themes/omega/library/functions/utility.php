...
add_action( 'init', 'omega_add_post_type_support' );

/* Filters the title for untitled posts. */
add_filter( 'the_title', 'omega_untitled_post' );
 ...
 * @return void
 */
function omega_add_post_type_support() {
 ...
 */
function omega_has_post_template( $template = '' ) {
 ...
 * @return string
 */
function omega_untitled_post( $title ) {

	if ( empty( $title ) && !is_singular() && in_the_loop() && !is_admin() )
		$title = __( '(Untitled)', 'omega' );
 ...
 * @return string
 */
function omega_locate_theme_file( $file_names ) {
