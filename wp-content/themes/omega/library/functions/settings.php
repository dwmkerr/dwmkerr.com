 ...
 * @uses   omega_get_prefix() Gets the prefix of the theme.
...
function omega_get_setting( $option = '' ) {
	...
	if ( !isset( $omega->settings ) || !is_array( $omega->settings ) )
		$omega->settings = get_option( omega_get_prefix() . '_theme_settings', $defaults );
...
function omega_get_default_theme_settings() {
	return apply_filters( omega_get_prefix() . '_default_theme_settings', array() );
}
